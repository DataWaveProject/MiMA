...
use time_manager_mod,       only:  time_manager_init, time_type, get_date
...
!   variables for forpy
!-------------------------------------------------------------------
integer :: ie, n_output=40
!type(ndarray) :: X_for_nd, Y_out_nd
...
if (use_forpy) then
    allocate(X3(idf*jdf,4,kmax)) ![U,V,W,T]
    allocate(X2(idf*jdf, 3)) ![lat,lon, psfc]
    allocate(Y_out(idf, jdf, n_output))
    ie = forpy_initialize()
...
subroutine cg_drag_calc (is, js, lat, pfull, zfull, wg_full, psfc, temp, uuu, vvv,  &
...
real, dimension(:,:,:), intent(in)      :: pfull, zfull, wg_full, temp, uuu, vvv
type(time_type),        intent(in)      :: Time
real           ,        intent(in)      :: delt
real, dimension(:,:,:), intent(out)     :: gwfcng_x, gwfcng_y
! real, dimension(:,:,:), asynchronous, intent(out)     :: gwfcng_x, gwfcng_y !forpy modification
!-------------------------------------------------------------------
!    intent(in) variables:
!
...
!---------------------------------------------------------------------
      ...
      if (cgdrag_alarm <= 0) then
          ...
          if (use_forpy) then
              ...
              ! print *, 'cg_drag: start make model input'
              call make_model_input(uuu, vvv, wg_full, psfc, temp, lat, X3, X2)
              ! print *, 'cg_drag: model inputs made'
              gwfcng_x = 0.0
              gwfcng_y = 0.0
              ! For now, we only use forpy for zonal drags.
              ! print *, 'cg_drag: start get_model_prediction'
              call get_model_prediction(run_emulator, model, X3, X2, Y_out)
              ! call get_model_prediction(run_emulator, model, X3, X2, gwfcng_x(:,:,1:n_output))
              gwfcng_x(:,:,1:n_output)=Y_out(:,:,:)
              ! print *, 'value copied over:', gwfcng_x(1,1,1)
              ! print *, 'cg_drag: done get_model_prediction'
              ! deallocate(X3)
              ! deallocate(X2)
              ! deallocate(Y_out)
          ! else
...
! forpy subroutine for making model inputs.
subroutine make_model_input(uuu, vvv, wg_full, psfc, temp, lat, X3, X2)
    ! We reshape 3d arrays uuu, vvv, wg_full, temp (imax,jmax,40), 
    ! to 2-dimensional arrays and stack them into X3 (bs=imax*jmax, 40,4)
    ! And reshape 2d array SP, 1d array, lat into 2D array X2(bs=imax*jmax, 3).
    ! Lon is put in to X2 in the python script run_emulator. 
    real, dimension(:,:,:), intent(in)    :: uuu, vvv, wg_full, temp
    real, dimension(:,:),   intent(in)    :: lat, psfc
    ! real, dimension(:,:),   intent(in)    :: ps
    real, dimension(:,:,:), intent(inout) :: X3
    real, dimension(:,:),   intent(inout) :: X2

    ! local variables
    integer :: j, imax, jmax, n_input
    real                    :: pif = 3.14159265358979/180.
    real                    :: pifinv = 180./3.14159265358979

    ! uuu is of dimension (imax,jmax,40)
    imax = size(uuu, 1)
    jmax = size(uuu, 2)
    do j=1,jmax
        ! 3d variables
        X3((j-1)*imax+1:j*imax,1,:) = uuu(:,j,:)
        X3((j-1)*imax+1:j*imax,2,:) = vvv(:,j,:)
        X3((j-1)*imax+1:j*imax,3,:) = wg_full(:,j,:)
        X3((j-1)*imax+1:j*imax,4,:) = temp(:,j,:)

        ! 2d variables
        X2((j-1)*imax+1:j*imax,1) = lat(:,j)*pifinv
        X2((j-1)*imax+1:j*imax,3) = psfc(:,j)/100
    end do
    ! print *, 'lat', X2(:,1)
    ! print *, 'sp', X2(:,3)
...
! forpy subroutine for getting model prediction.

subroutine get_model_prediction(run_emulator, model, X3, X2, Y_out)
    real, dimension(:,:,:), intent(in)  :: X3
    real, dimension(:,:),   intent(in)  :: X2
    type(module_py)       , intent(in)  :: run_emulator
    type(object)          , intent(in)  :: model
    real, dimension(:,:,:), asynchronous, intent(inout) :: Y_out

    ! local variables
    integer, parameter :: imax = 128, kmax = 40
    integer            :: num_col!, ie
    type(ndarray)      :: X3_nd, X2_nd, Y_out_nd
    ! type(object)       :: Y_out_obj
    type(tuple)        :: args
    ! real,dimension(:,:), pointer :: matrix

    num_col = size(X3,1)/imax
    ! creates numpy arrays
    ie = ndarray_create_nocopy(X3_nd, X3)
    ie = ndarray_create_nocopy(X2_nd, X2)
    ie = ndarray_create_nocopy(Y_out_nd, Y_out)

    ! run through model
    ie = tuple_create(args,5)
    ie = args%setitem(0,model)
    ie = args%setitem(1,X3_nd)
    ie = args%setitem(2,X2_nd)
    ie = args%setitem(3,Y_out_nd)
    ie = args%setitem(4,num_col)
    ...
    ! ie = print_py(Y_out_nd)
    ! Print *, 'Value to copy over', Y_out(1,1,1)
