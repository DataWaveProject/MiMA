...
class model(nn.Module):
    def __init__(self, s_dict,n_levels=40):
        ...
        self.lon=s_dict['lon']; self.lat = s_dict['lat'];
        self.n_flat_in = int(self.n_ch[3]*n_levels/2)+3
        self.n_flat_out = int(self.n_ch[3]*n_levels/2)
        self.input_stats = {'mean_3': s_dict['input_3d_mean'], 'std_3': s_dict['input_3d_std'], 
                            'mean_sp': s_dict['sp_mean'], 'std_sp': s_dict['sp_std']}
        ...
        self.cnn_encode = nn.Sequential(
            nn.Conv1d(self.n_ch[0], self.n_ch[1], self.kernel_size, 1, padding='same', dilation=self.n_dilations[0]), nn.ELU(),
            nn.Conv1d(self.n_ch[1], self.n_ch[2], self.kernel_size, 1, padding='same', dilation=self.n_dilations[1]), nn.ELU(),
            nn.Conv1d(self.n_ch[2], self.n_ch[3], self.kernel_size, 1, padding='same', dilation=self.n_dilations[2]), nn.ELU(),
        ...
        self.cnn_decode = nn.Sequential(
            nn.ConvTranspose1d(self.n_ch[3], self.n_ch[2], self.kernel_size, 2, padding=1, output_padding=1, dilation=self.n_dilations[3]), nn.ELU(),
            nn.ConvTranspose1d(self.n_ch[2], self.n_ch[1], self.kernel_size, 1, padding=1, dilation=self.n_dilations[4]), nn.ELU(),
            nn.ConvTranspose1d(self.n_ch[1], 1,          self.kernel_size, 1, padding=1, dilation=self.n_dilations[5])
        )
    ...
    def forward(self, x):
        ...
        u_range = tensor(array([max(x3[l,0,:self.source_level[whichlat[l]]])-min(x3[l,0,:self.source_level[whichlat[l]]]) for l in range(batchsize)])); del whichlat
        i = searchsorted(self.bin_edges, u_range, side='left'); del u_range; 

        # Standardize
        x3 -= reshape(self.input_stats['mean_3'],(1,4,1)); x3 /= reshape(self.input_stats['std_3'],(1,4,1));
        xloc[:,-1] -= self.input_stats['mean_sp']; xloc[:,-1]/= self.input_stats['std_sp']
        ...
        # Unstandardize
        gu *= self.output_stats['std']; gu += self.output_stats['mean']
        gu -= self.sm_mean[i,:].astype(float32); del i
        return gu
