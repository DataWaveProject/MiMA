'''
This python script outlines the forpy commands that will be done in cg_drag_lmy.f90
In the correct folder, place module named model_arch and the model weights h5 file.
This additionally removes the bias w.r.t. the shear metric estimated on the training data. 
...
temp = np.random.randn(128*2, 40).astype(np.float32)
...
def initialize():
    ...
    best_model = load(dirpath.joinpath("model_weights_ae98.h5"), map_location=device(device_str))
...
def compute_reshape_drag(*args):
    ...
    X3 = from_numpy(X3.astype(float32))
    X2 = from_numpy(X2.astype(float32))

    with no_grad():
        temp = model([X3,X2])
    # temp =temp.double()- model.sm_mean[i,:] # remove mean bias of the corresponding sample urange bin statistic
...
if __name__ == "__main__":
    ...
    # X2 = zeros(10) + 1.0
    # X3 = zeros(10) + 1.0
    Yo = compute_reshape_drag(trained_model, X3, X2, Y_out, 2)
